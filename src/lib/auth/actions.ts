...
// ============================================================
// SIGN UP — Cadastro com email, senha e token de convite (opcional)
// ============================================================
export async function signUp(formData: FormData) {
  ...
  // Se veio token de convite, validar antes de criar a conta
  if (token && token.trim() !== '') {
    const { data: validation } = await supabase
      .rpc('validate_invite_token', { p_token: token.trim() })
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    options: {
      data: {
        name,
        // Armazenar token no metadata para usar no callback
  ...
  // Se o token existe e o usuário foi criado, vincular ao pending_profile
  ...
  // Se confirmação de email não for necessária (configuração do Supabase),
  // redirecionar para o início. Caso contrário, informar para verificar email.
  if (data.session) {
    revalidatePath('/', 'layout')
    redirect('/inicio')
  }
...
// ============================================================
// RESET PASSWORD — Envia email de recuperação
// ============================================================
...
// ============================================================
// UPDATE PASSWORD — Atualiza a senha após link de recuperação
// ============================================================
...
// ============================================================
// GET SESSION — Retorna sessão atual (uso em Server Components)
// ============================================================
...
// ============================================================
// GET USER PROFILE — Retorna perfil do usuário logado
// ============================================================
...
// ============================================================
// LINK INVITE — Vincula token de convite ao usuário logado
// Útil se o usuário se cadastrou sem token e quer vincular depois
// ============================================================
export async function linkInviteToken(token: string) {
  ...
  const { data, error } = await supabase.rpc('link_pending_profile', {
    p_user_id:      user.id,
    p_invite_token: token.trim(),
  })
...
// ============================================================
// UPDATE PROFILE — Atualiza dados de texto do perfil
// ============================================================
export async function updateProfile(formData: FormData) {
  ...
  // username: apenas letras, números, _ e . (sem espaços)
  ...
  // Se informou username, verificar se já está em uso por outro usuário
  if (username) {
...
// ============================================================
// UPLOAD AVATAR — Faz upload para o bucket avatars e salva URL
// ============================================================
export async function uploadAvatar(formData: FormData) {
  ...
  // Gerar nome único para evitar cache de URL antiga
